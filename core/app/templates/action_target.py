    ...
    for module in range (len(modules())):
            if request.form.get(f'{modules()[module].title()}') == f'{modules()[module].title()}':
                ...
                if request.form.get('Server') == 'Server':
                    additional = """<input type="text" name="Target_name" placeholder="target name"><input type="text" name="IP" placeholder="IP"><input type="text" name="Resource_type" placeholder="resource type">"""    
                else:
